...
func (ug *UseCaseGenerator) genDeleteUCAST() *ast.FuncDecl {
	return &ast.FuncDecl{
		Recv: &ast.FieldList{
			List: []*ast.Field{
				{
		...
		Name: ast.NewIdent(strings.Join([]string{"Delete", ug.entAST.Name}, "")),
		Type: &ast.FuncType{
					...
					Results: []ast.Expr{
						&ast.CallExpr{
							Fun: &ast.SelectorExpr{
								...
								Sel: ast.NewIdent(strings.Join([]string{"Delete", ug.entAST.Name}, "")),
							},
							Args: []ast.Expr{
								0: ast.NewIdent("ctx"),
