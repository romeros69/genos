...
func (cg *ControllerHTTPGenerator) genCreateControllerAST() *ast.FuncDecl {
	return &ast.FuncDecl{
		...
		Body: &ast.BlockStmt{
			List: []ast.Stmt{
				0: cg.genCallServiceCreate(),
				1: cg.genCheckCallServiceCreate(),
				2: cg.genDeclResponseCreate(),
				3: cg.genCheckNilResponseCreate(),
...
func (cg *ControllerHTTPGenerator) genCallServiceCreate() *ast.AssignStmt {
	return &ast.AssignStmt{
		Lhs: []ast.Expr{
			0: ast.NewIdent(cg.entAST.Name + "List"),
			1: ast.NewIdent("err"),
		},
		Tok: token.DEFINE,
		Rhs: []ast.Expr{
...
func (cg *ControllerHTTPGenerator) genDeclResponseCreate() *ast.DeclStmt {
	return &ast.DeclStmt{
		Decl: &ast.GenDecl{
			Tok: token.VAR,
			Specs: []ast.Spec{
...
func (cg *ControllerHTTPGenerator) genCheckNilResponseCreate() *ast.IfStmt {
	return &ast.IfStmt{
		Cond: &ast.BinaryExpr{
			X:  ast.NewIdent(strings.ToLower(cg.entAST.Name) + "List"),
			Op: token.NEQ,
			Y:  ast.NewIdent("nil"),
		},
		Body: &ast.BlockStmt{
			List: []ast.Stmt{
				&ast.AssignStmt{
					...
					Tok: token.ASSIGN,
					Rhs: []ast.Expr{
						ast.NewIdent(strings.ToLower(cg.entAST.Name) + "List"),
					},
				},
			},
		},
